...
public class GlobalExceptionHandler {
  ...
  ResponseEntity<ProblemDetail> handleNotFound(TrainerNotFoundException ex) {
    ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
    return ResponseEntity.status(HttpStatus.NOT_FOUND).body(problem);
  }

  @ExceptionHandler(OwnedPokemonNotFoundException.class)
  ResponseEntity<ProblemDetail> handlePokemonNotFound(OwnedPokemonNotFoundException ex) {
    ProblemDetail problem = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
    return ResponseEntity.status(HttpStatus.NOT_FOUND).body(problem);
  }
